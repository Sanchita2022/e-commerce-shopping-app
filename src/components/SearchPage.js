...
const common_data_api = "https://phpwebdevelopmentservices.com/project-react-backend/api/common-data";
...
const SearchPage = ({AppState:[
 ...
 const [resultState, setResultState] = useState({found:false,result:dummy_result});
 const result = resultState.result;
 const [currentCategory, setCurrentCategory] = React.useState(result.categories[0]);
 ...
 <option key={category.name} onClick={() => handleCategorySelect(category.name.id)}>{category.name}</option>
 ));
 // Create dropdown menu list for selecting Sub Category of Product
 const sub_categories_options = currentCategory.sub_categories.map((sub_category) => (
 <li key={sub_category.id}>
 <label className="contect_container_checkBox">{sub_category.name}
 <input type="checkbox" defaultChecked={sub_category.status==1?"checked":"unchecked"} name="text"/>
 ...
 pageList.push(
 <li key='page_prev'>
 <a href='#' onClick={e=>setCurrentPage((currentPage > 0) ? (currentPage - 1): 0)}>
 ...
 if(i == 0 || i == pageCount-1 || Math.abs(currentPage-i-1) < itemsPerPage){
 ...
 pageList.push(
 <li key={`page${i}`}>
 <a href="#" onClick={e=>setCurrentPage(i)}>{i+1}</a>
 </li>
 )
 ...
 pageList.push(
 <li key='page_next'>
 <a href='#' onClick={e=>setCurrentPage((currentPage > pageCount-1) ? (pageCount - 1): currentPage)}>
 ...
 const selectedCategory = result.categories.find((category) => category.id === categoryId);
 ...
 <input type="text" placeholder="Keyword" className="search-input" defaultValue={keyword}></input>
 ...
 <input type="text" className="price_numb" readOnly id="amount" value={`Rs.${priceRange[0]} - Rs.${priceRange[1]}`}></input>
 </span>
 </div>
 </div>
 ...
 <label className="event-sort">Showing {itemStartIndex}-{itemEndIndex} out of {totalItems} product for {currentCategory.name}</label>
